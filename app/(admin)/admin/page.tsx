...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-10 font-sans dir-rtl text-zinc-900 dark:text-white" dir="rtl">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-zinc-900 dark:text-white">نظرة عامة والتحليلات الحية 🐺</h1>
        ...
        {statCards.map((card, i) => {
          ...
          return (
            ...
            >
              <Link
                href={card.href}
                className="block bg-white dark:bg-zinc-950 rounded-2xl p-6 border border-zinc-200 dark:border-zinc-800 hover:border-amber-500/50 shadow-sm dark:shadow-2xl transition-all duration-300 group"
              >
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-bold text-zinc-600 dark:text-zinc-400 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
                    {card.title}
                  </span>
                ...
                <div className="text-2xl font-black text-zinc-900 dark:text-white mb-2 tracking-tight">
                  {card.value}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div
          className="lg:col-span-2 bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm dark:shadow-2xl"
        ...
        {/* Status Donut Chart */}
        <motion.div
          className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm dark:shadow-2xl"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {topProducts.length > 0 && (
        <motion.div
          className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm dark:shadow-2xl"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-4 mb-6">
            <div>
              <h2 className="text-base font-black text-zinc-900 dark:text-white">🏆 أكثر المنتجات مبيعاً</h2>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">أعلى 5 منتجات من حيث عدد الطلبات</p>
            </div>
          </div>
                ...
                <div key={p.name} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 rounded-full bg-amber-500/20 border border-amber-500/30 flex items-center justify-center text-amber-600 dark:text-amber-400 font-black text-[10px] shrink-0">
                        {i + 1}
                      </span>
                      <span className="text-zinc-900 dark:text-white font-bold truncate max-w-[160px]">{p.name}</span>
                    </div>
                    <div className="flex items-center gap-3 text-right shrink-0">
                      <span className="text-zinc-500 dark:text-zinc-400">{p.count} قطعة</span>
                      <span className="text-amber-600 dark:text-amber-400 font-black">{formatPrice(p.revenue)}</span>
                    </div>
                  </div>
      ...
      <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm dark:shadow-2xl space-y-6">
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-4">
          <div>
            <h2 className="text-lg font-black text-zinc-900 dark:text-white">أحدث الطلبات المستلمة</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">آخر الطلبات التي تم إنشاؤها عبر المتجر</p>
          </div>
          <Link
            href="/admin/orders"
        ...
        {recentOrders.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right text-xs">
              <thead>
                <tr className="border-b border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 font-bold uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60">
                {recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-black text-amber-700 dark:text-amber-300">
                      {formatPrice(order.total)}
                    </td>
                    <td className="py-3.5 px-4">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200">
